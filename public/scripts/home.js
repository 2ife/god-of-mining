...
const minerSendModalBtns =
  minersContainer.querySelectorAll(".minerSendModalBtn");
const minerGenerateBtnContainer = gamePart.querySelector(
  "#minerGenerateBtnContainer"
);
const minerGeneraterByCash_amountInput =
  minerGenerateBtnContainer.querySelector("#minerGeneraterByCash_amountInput");
...
const minerGeneraterByCoin_amountInput =
  minerGenerateBtnContainer.querySelector("#minerGeneraterByCoin_amountInput");
...
const minerUpgradeModal_closeBtn =
  minerUpgradeModal.querySelector(".modal_closeBtn");
...
const getSendLogs = (nextSendLogs) => {
  ...
  nextSendLogs.forEach((log) => {
    ...
    sendLogList.innerHTML += `<br><br>발송인: ${sender}<br>수령인: ${receiver}<br>채굴기 레벨: ${minerLevel}<br>채굴기 수량: ${minerAmounts}<br>${new Date(
      time
...
const checkLoginCode = async () => {
  try {
    ...
    if (mine) {
      alertByModal(
        `${user.minersTotalPerformance.toLocaleString("ko-KR")} 코인 채굴!`
      );
    }
  } catch (err) {
    localStorage.removeItem("LOGIN_CODE");
...
const login = async (event) => {
  ...
  try {
    ...
    showLoading();
    const res = await axios.default.post("/auth/login", {
      loginId: id,
      password,
    });
    const { data } = res;
...
const generateMiner = async (type) => {
  try {
    ...
    if (
      ...
      return alertByModal(
        `현재 Lv1 채굴기 생성 가능 개수: 1~${32 - lv1MinerAmounts}`
      );
    }
    if (type === "cash" && user.cash < generateAmounts * LV1_MINER_CASH) {
      return alertByModal(
        `캐시가 부족합니다. (필요 캐시: ${(
          generateAmounts * LV1_MINER_CASH
        ).toLocaleString("ko-KR")})`
      );
    } else if (
      type === "coin" &&
      ...
      return alertByModal(
        `코인이 부족합니다. (필요 코인: ${(
          generateAmounts * LV1_MINER_COIN
        ).toLocaleString("ko-KR")})`
      );
    }
    showLoading();
...
const sendMiner = async () => {
  try {
    ...
    if (receiver_max === 0 || receiver_max) {
      return alertByModal(
        `현재 해당 유저에게 LV${targetMiner.level} 채굴기는 최대 ${receiver_max}개까지만 선물 가능합니다.`
      );
    }
    const now = Date.now();
    sendLogs.push({
    ...
    sendLogList.innerHTML = `${sendLogList.innerHTML.slice(
      0,
      sendLogList.innerHTML.indexOf("선물 기록\n") + 6
    )}<br><br>발송인: ${user.nick}<br>수령인: ${receiverNick}<br>채굴기 단계: ${
      targetMiner.level
    }<br>채굴기 수량: ${sendAmounts}<br>${new Date(now).toLocaleString(
      "ko-KR"
